#!/usr/bin/env node
// plan.js — lee pins.yml, resuelve cada pin y emite el plan de bumps.
// En dry-run imprime la tabla y no toca ningun repo.
...
// Lee el valor vigente del ARG directo del repo: el Dockerfile es la fuente de
// verdad, no una copia en este archivo que se desincroniza.
...
for (const r of cfg.repos) for (const f of r.files) for (const p of f.pins) {
  ...
  if (raw === null) { plan.push({ ...p, repo: r.repo, path: f.path, status: 'ARG_NO_ENCONTRADO' }); continue; }
...
console.log('\n' + W('REPO', 26) + W('ARG', 17) + W('ACTUAL', 10) + W('ESTADO', 22) + W('PROPUESTO', 11) + 'QUEDAN');
console.log('-'.repeat(96));
for (const p of plan) {
  const target = p.target ? (p.prefix || '') + p.target : '-';
  const left = p.status === 'YA_LIMPIA' ? '0' : (p.remaining ?? p.current_findings ?? '-');
  console.log(W(p.repo, 26) + W(p.arg, 17) + W(p.current, 10) + W(p.status, 22) + W(target, 11) + left);
}
const prs = plan.filter(p => p.status === 'BUMP' || p.status === 'BUMP_PARCIAL');
console.log(`\nPRs que abriria: ${prs.length}`);
